...
export const postRouter = router({
  create: loggedProcedure
    ...
    .mutation(async ({ ctx: { user }, input: { title, content, tags } }) => {
  ...
  setBookmarkState: loggedProcedure
    ...
    .mutation(async ({ ctx: { user }, input: { postId, bookmarkState } }) => {
      const post = await db.post.findFirst({
        where: {
          id: postId,
          OR: [
  ...
  delete: loggedProcedure
    .input(z.object({ postId: z.string().uuid("La publicación no existe") }))
    .mutation(
      async ({
        ctx: {
          user: { id: userId },
        ...
        if (post.authorId !== user.id && !user.isAdmin && !user.isModerator) {
          throw new TRPCError({
